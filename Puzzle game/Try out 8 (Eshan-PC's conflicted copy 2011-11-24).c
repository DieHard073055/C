...
char read_records();
char write_records(char Name[50],char score[20]);
int Get_Ran();
...
char start_game()
{
	int axis_X, axis_Y, User_Decision_Exit,score;
	char timer = 'F';
	char s_score[20];
	time_t start,end;
	board A[b_][b_];
	...
	do
	{
		...
		User_Decision_Exit = input(A);
		if (User_Decision_Exit == 'F')
		{
			if(timer == 'T')
			{
				end = time(NULL);
...
char user_input(char menu[4])
{
	...
	User_Input = getch();
	if ( User_Input == 87 || User_Input == 119 )
	{
		if ( axis_Y > 0 )
		{
			menu[axis_Y] = 'x';
	...
	else if ( User_Input == 83 || User_Input == 115 )
	{
		if (axis_Y < 4)
		{
			menu[axis_Y] = 'x';
...
char option()
{
	...
	do
	{
		if (D == 1)
		{
			strcpy(Dificlty_Choice, "Easy");
			symbol = 176;
			b_ = 3;
		}
		else if (D == 2)
		{
			strcpy(Dificlty_Choice, "Normal");
			symbol = 177;
			b_ = 5;
		}
		else if (D == 3)
		{
			strcpy(Dificlty_Choice, "Hard");
			symbol = 178;
			b_ = 7;
		}
		system("cls");
